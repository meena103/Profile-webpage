...
from .models import Post, PostComment, course, pdfview
...
def EEL1010(request):
    cours = course.objects.get(cours = "EEL1010")
    pdf = pdfview.objects.all().filter(course = cours)

    data = {"pdfs": pdf}
    return render(request,'pdf_list.html',data)

def CT(request):
    cours = course.objects.get(cours = "Circuit Theory")
    pdf = pdfview.objects.all().filter(course = cours)
    data = {"pdfs": pdf}
...
def RE(request):
    cours = course.objects.get(cours = "Radar Engineering")
    pdf = pdfview.objects.all().filter(course = cours)
    data = {"pdfs": pdf}
...
def MVT(request):
    cours = course.objects.get(cours = "Millimeter Wave Technology")
    pdf = pdfview.objects.all().filter(course = cours)
    data = {"pdfs": pdf}
    ...
    return render(request,'pdf_list.html',data)
def ME(request):
    cours = course.objects.get(cours = "Microwave Engineering")
    pdf = pdfview.objects.all().filter(course = cours)
    data = {"pdfs": pdf}
